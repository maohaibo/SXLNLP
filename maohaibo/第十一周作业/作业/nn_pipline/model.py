...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        class_num = vocab_size #config["class_num"]
        ...
        if model_type == "fast_text":
            ...
            self.use_bert = True
            self.encoder = BertModel.from_pretrained(config["pretrain_model_path"], return_dict=False, attn_implementation="eager")
    ...
    def forward(self, x, mask=None, target=None):
        if target is not None:
            ...
            print('predict.view(-1, predict.shape[-1]):', predict.view(-1, predict.shape[-1]).shape)
            print('target.view(-1):', target.view(-1).shape)
            '''
            torch.Size([104, 4623])
            torch.Size([104, 30])
            '''
